...
# IMPORTANT: Import multiprocessing
...
# ----------------------------------------------------------------------------
# STAGE 0: CACHE HEAVY RESOURCES (Embedding Model Only)
# ----------------------------------------------------------------------------
@st.cache_resource
...
# NOTE: The PII model will now be loaded by each worker process, not cached by Streamlit.
...
# ----------------------------------------------------------------------------
# This global variable will hold the model for each individual worker process
pii_model_worker = None

def init_worker():
    """Initializer for each worker process: loads its own copy of the PII model."""
    global pii_model_worker
    logging.info("Initializing worker process and loading PII model...")
    # Replace with your actual model path if necessary
    pii_model_worker = pipeline("token-classification", "Jean-Baptiste/roberta-large-ner-english", aggregation_strategy="first", device=-1)

def process_chunk_with_pii(args):
    """
    The function that each worker process will execute.
    It performs PII masking and then calls the LLM for tagging.
    """
    chunk_id, chunk_text = args
    if not chunk_text or len(chunk_text.strip()) < 20: return {}

    # The worker uses its own pre-loaded model, preventing race conditions
    masked_chunk, has_pii = mask_pii_robust(chunk_text, pii_model_worker)

    prompt = f"""For the text chunk with ID '{chunk_id}', create a JSON object...
    # Text Chunk to Analyze: {masked_chunk}"""

    # In a real scenario, your LLM call would go here. We use the mock for now.
...
# --- Other functions (abc_response, etc.) are included for completeness ---
...
def generate_hierarchical_graph(tagged_data: List[Dict], doc_title: str) -> Dict:
    valid_tagged_data = [item for item in tagged_data if item]
...
def main():
    ...
    with st.sidebar:
        ...
        user_text = ""
        if input_type == "Text":
            user_text = st.text_area("Paste your document text here:", value="Dr. Sarah Miller's email is sarah.miller@example.com. Another chunk has no PII.", height=300)
        else:
        ...
        if st.button("Generate Diagram", type="primary", use_container_width=True, disabled=not user_text):
            with st.spinner("Processing document (including PII scan)..."):
                ...
                # --- FIX: Use a multiprocessing Pool for safe and fast parallel processing ---
                tasks = [(f"chunk-{i}", chunk) for i, chunk in enumerate(chunks)]
                # initializer is called once per worker process to load the model safely
                with Pool(processes=4, initializer=init_worker) as pool:
                    tagged_data_results = pool.map(process_chunk_with_pii, tasks)
...
# This "if" block is essential for multiprocessing to work correctly
