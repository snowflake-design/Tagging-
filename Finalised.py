...
if st.session_state.processing_complete and st.session_state.graph_data:
    ...
    # Display the flow diagram with proper key to prevent refresh
    ...
    # Update the flow state and handle node selection
    if flow_result and flow_result.selected_id:
        current_selected = flow_result.selected_id
        
        # Only update if different node selected
        if current_selected != st.session_state.last_selected_id:
            st.session_state.last_selected_id = current_selected
