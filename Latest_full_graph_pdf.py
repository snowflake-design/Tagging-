...
# ----------------------------------------------------------------------------
# NEW: PDF TEXT EXTRACTION FUNCTION (Adapted for Streamlit)
# ----------------------------------------------------------------------------
def extract_text_from_pdf(file_buffer) -> str:
    ...
    full_text = ""
    try:
        # PyPDF2 can read directly from the file-like object
...
with st.sidebar:
    ...
    # --- UPDATED: ADDED FILE UPLOADER FOR PDFS ---
...
"""
    user_text = st.text_area("Paste your document text here:", value=DEFAULT_TEXT, height=250, key="user_text_area")

    if st.button("Generate Diagram", type="primary", use_container_width=True):
        # --- UPDATED: LOGIC TO HANDLE EITHER PDF OR TEXT ---
        text_to_process = ""
        doc_title = "Document Analysis"

        if uploaded_file is not None:
            text_to_process = extract_text_from_pdf(uploaded_file)
            doc_title = uploaded_file.name # Use PDF name as title
        elif user_text:
        ...
        if text_to_process:
            ...
            st.warning("Please upload a PDF or paste some text to generate a diagram.")


if st.session_state.graph_data:
    st.header("Topic Hierarchy Diagram")
    
    st.session_state.flow_state = streamlit_flow('tree_layout', st.session_state.flow_state, layout=TreeLayout(direction='down'), fit_view=True, get_node_on_click=True, height=600)
