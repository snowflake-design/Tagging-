...
def init_session_state():
    ...
    if 'current_content_hash' not in st.session_state:
        st.session_state.current_content_hash = None
    if 'current_input_method' not in st.session_state:
        st.session_state.current_input_method = None
    if 'cached_results' not in st.session_state:
        st.session_state.cached_results = {}

def generate_content_hash(text: str, input_method: str, doc_title: str = "") -> str:
    """Generate SHA-256 hash of content including input method and title for proper caching"""
    content_string = f"{input_method}::{doc_title}::{text}"
...
def cache_results(content_hash: str, graph_data: Dict, tagged_data: List[Dict], flow_state: StreamlitFlowState, input_method: str, doc_title: str):
...
def load_cached_results(content_hash: str) -> bool:
    ...
    if content_hash in st.session_state.cached_results:
        ...
        st.session_state.current_input_method = cached['input_method']
        st.session_state.processing_complete = True
        return True
    return False

def clear_current_results():
    """Clear current processing results but keep cache"""
    st.session_state.graph_data = None
    st.session_state.tagged_data = None
    st.session_state.flow_state = StreamlitFlowState(nodes=[], edges=[])
    st.session_state.processing_complete = False
...
def should_clear_results(new_input_method: str) -> bool:
    """Check if results should be cleared due to input method change"""
    return (st.session_state.current_input_method is not None and 
            st.session_state.current_input_method != new_input_method)
...
# Processing function
def process_document(text_to_process: str, doc_title: str, input_method: str, page_count: int = 0):
    """Process document and update session state with proper caching"""
    try:
        # Generate content hash including input method and title for proper separation
        content_hash = generate_content_hash(text_to_process, input_method, doc_title)
        ...
        with st.spinner(f"Processing document... {'This may take longer for larger PDFs.' if page_count > 0 else 'Please wait.'}"):
            ...
            # Update session state and cache results
            ...
            cache_results(content_hash, graph_data, tagged_data, flow_state, input_method, doc_title)
...
with st.sidebar:
    ...
    if should_clear_results(input_method):
    ...
    if input_method == "PDF Upload":
        ...
        # Clear results if a different PDF is uploaded
        if uploaded_file is not None:
            current_file_key = f"{uploaded_file.name}_{uploaded_file.size}"
            if 'current_pdf_key' not in st.session_state:
                st.session_state.current_pdf_key = None
            
            # If this is a different PDF than currently processed, clear results
            if (st.session_state.current_pdf_key != current_file_key and 
                st.session_state.processing_complete and 
                st.session_state.current_input_method == "PDF Upload"):
                clear_current_results()
                st.info("New PDF detected. Previous results cleared.")
            
            st.session_state.current_pdf_key = current_file_key
        ...
        if uploaded_file is not None:
            ...
            # No file uploaded, clear PDF key
        ...
        st.markdown('</div>', unsafe_allow_html=True)
            ...
            # Clear results if text content changes significantly
            if 'current_text_content' not in st.session_state:
                st.session_state.current_text_content = None
                
            if (st.session_state.current_text_content != user_text and 
                st.session_state.processing_complete and 
                st.session_state.current_input_method == "Text Input" and
                st.session_state.current_text_content is not None):
                clear_current_results()
                st.info("Text content changed. Previous results cleared.")
...
"""
    ...
    if st.button("Generate Topic Flow Diagram", type="primary", use_container_width=True):
        ...
        if text_to_process:
            process_document(text_to_process, doc_title, input_method, page_count)
        else:
            if input_method == "PDF Upload":
    ...
    if st.session_state.cached_results:
        ...
        for hash_key, cached_item in st.session_state.cached_results.items():
            input_type = cached_item.get('input_method', 'Unknown')
            doc_name = cached_item.get('doc_title', 'Untitled')
            timestamp = time.strftime('%H:%M:%S', time.localtime(cached_item.get('timestamp', 0)))
            st.text(f"• {input_type}: {doc_name} ({timestamp})")
...
# Main content area - only show if current session has valid results
if (st.session_state.processing_complete and 
    st.session_state.graph_data and 
    st.session_state.current_input_method == input_method):
