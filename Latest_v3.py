...
def load_embedding_model():
    ...
    """
    # NOTE: Removed st.toast() from this cached function to prevent CacheReplayClosureError.
...
def abc_response(prompt: str) -> str:
    ...
    """
    # This function is kept for reference. In your real app, you would
    # replace the call to this function with your actual LLM API call.
    ...
    if "You are an expert Information Architect" in prompt:
        ...
        "nodes": [
            ...
            {"id": "Artificial_Intelligence", "label": "Artificial Intelligence", "group": "parent"},
            {"id": "Environmental_Issues", "label": "Environmental Issues", "group": "parent"},
            {"id": "AI_Dev_Ethics", "label": "AI Development & Ethics", "group": "child"},
            {"id": "Climate_Change_Response", "label": "Climate Change & Response", "group": "child"},
        ...
        "edges": [
            {"source": "Document Analysis", "target": "Artificial_Intelligence"},
            {"source": "Document Analysis", "target": "Environmental_Issues"},
            {"source": "Artificial_Intelligence", "target": "AI_Dev_Ethics"},
            {"source": "Environmental_Issues", "target": "Climate_Change_Response"},
            {"source": "AI_Dev_Ethics", "target": "chunk-0"},
            {"source": "AI_Dev_Ethics", "target": "chunk-1"},
            {"source": "Climate_Change_Response", "target": "chunk-2"},
            {"source": "Climate_Change_Response", "target": "chunk-3"}
        ],
        "consolidation_map": {
            "AI_Dev_Ethics": ["chunk-0", "chunk-1"],
            "Climate_Change_Response": ["chunk-2", "chunk-3"]
        }
        })
    return "{}"
...
def generate_topic_for_chunk(chunk_with_id: tuple) -> Dict:
    ...
    {chunk_text}
    """
    # In your real code, replace abc_response with your actual LLM call
...
def generate_hierarchical_graph(tagged_data: List[Dict], doc_title: str) -> Dict:
    ...
    # --- PROMPT FIX ---
    # This prompt is now more explicit to guide the LLM into producing a consistent structure.
    prompt = f"""
    ...
    - For 'parent' and 'child' nodes, you MUST invent a simple, unique `id` with no spaces or special characters (e.g., 'Category_1', 'Consolidated_Topic_A'). The `label` can be more descriptive and human-readable.
    - The `consolidation_map` must use these exact simple `id`s as its keys.
    - Follow the format below precisely.

    ```json
    {{
      "nodes": [
        {{"id": "Root_Node_ID", "label": "Root Node Label", "group": "root"}},
        {{"id": "Parent_Category_1", "label": "Parent Category 1", "group": "parent"}},
        {{"id": "Consolidated_Topic_A", "label": "Consolidated Topic A", "group": "child"}},
        {{"id": "chunk-0", "label": "Original Topic 1's Label", "group": "grandchild"}}
      ],
      "edges": [
        {{"source": "Root_Node_ID", "target": "Parent_Category_1"}},
        {{"source": "Parent_Category_1", "target": "Consolidated_Topic_A"}},
        {{"source": "Consolidated_Topic_A", "target": "chunk-0"}}
      ],
      "consolidation_map": {{
        "Consolidated_Topic_A": ["chunk-0", "chunk-1"],
        "Consolidated_Topic_B": ["chunk-2"]
      }}
    }}
    ```
    ...
    if st.session_state.flow_state and st.session_state.flow_state.selected_id:
        ...
        with st.sidebar:
            ...
            if node_info:
                ...
                # --- DISPLAY LOGIC FIX ---
                # This block is now more robust and no longer depends on the consolidation_map.
                elif group == 'child':
                    st.info("This is a consolidated topic. Information below is aggregated from its source chunks.")
                    
                    # ROBUST METHOD: Find grandchildren by looking at the graph's edges.
                    grandchild_ids = [
                        edge['target'] for edge in st.session_state.graph_data['edges'] 
