...
# --- FIX: Define the specific PII types you want to mask ---
PII_ENTITY_TYPES = {'PER', 'LOC', 'ORG'} # A set for fast lookups (e.g., Person, Location, Organization)
...
def load_pii_model():
    ...
    return pipeline("token-classification", "dslim/bert-base-NER", aggregation_strategy="first", device=-1)

# ----------------------------------------------------------------------------
# PII MASKING MODULE
# ----------------------------------------------------------------------------
def mask_pii_robust(text: str, ner_pipeline, word_limit=128):
    """
    Detects and masks PII using a word-aware sliding window to prevent splitting words.
    """
    all_entities = []
    words = text.split(' ')

    for i in range(0, len(words), word_limit):
        word_window = words[i:i + word_limit]
        window_text = ' '.join(word_window)
        
        char_offset = len(' '.join(words[:i])) + 1 if i > 0 else 0
        
        if not window_text: break
            
        window_entities = ner_pipeline(window_text)
        
        for entity in window_entities:
            # FIX: Only consider entities that are in our PII list
            if entity['entity_group'] in PII_ENTITY_TYPES:
...
def abc_response(prompt: str) -> str:
    ...
    if "You are an expert Information Architect" in prompt:
        return json.dumps({
        "nodes": [{"id": "Document_Analysis", "label": "Document Analysis", "group": "root"}, {"id": "Inspections_and_Reports", "label": "Inspections and Reports", "group": "parent"}, {"id": "Inspection_Reports", "label": "Inspection Reports", "group": "child"}, {"id": "chunk-2", "label": "HVAC System Inspection Report", "group": "grandchild"}],
...
def create_flow_elements(graph_data: Dict, tagged_data: List[Dict]) -> (List[StreamlitFlowNode], List[StreamlitFlowEdge]):
    ...
    for node in graph_data.get('nodes', []):
        ...
        # FIX: Only highlight the child node, not the parent
        if group == 'child':
            ...
            if node_info:
                ...
                if group in ('parent', 'child'):
                    ...
                    if chunk_info:
                        ...
                        # FIX: Show both original and masked text
