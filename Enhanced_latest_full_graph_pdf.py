...
def cache_results(content_hash: str, graph_data: Dict, tagged_data: List[Dict], flow_state: StreamlitFlowState):
...
def process_document(text_to_process: str, doc_title: str, page_count: int = 0):
    """Process document and update session state with caching"""
    try:
        # Generate content hash
        content_hash = generate_content_hash(text_to_process)
        ...
        with st.spinner(f"Processing document... {'This may take longer for larger PDFs.' if page_count > 0 else 'Please wait.'}"):
            ...
            cache_results(content_hash, graph_data, tagged_data, flow_state)
